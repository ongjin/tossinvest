...
| 🚦 **클라이언트단 레이트리미터** | 그룹별 토큰버킷 + **9시 개장 10분 ORDER/ORDER_INFO 반토막**(6→3) 반영. |
...
| ✅ **라이브 키 없이 그린** | `git clone && uv sync && pytest` → **136개 테스트** 통과. 기여 장벽 0. |
...
uv run --package tossinvest-mcp pytest tossinvest-mcp/tests           # 91 passing
```
