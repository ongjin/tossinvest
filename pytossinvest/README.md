...
TossInvestClient(
    ...
    max_retries=3,             # 429 bounded 재시도 횟수 (0 이면 즉시 RateLimitError)
    retry_max_wait=60.0,       # 재시도 백오프 상한 (초)
    rng=random.random,         # jitter 주입 — 테스트 결정성용
)
```

`sleep`/`monotonic`/`now_kst` 주입은 **테스트 결정성**을 위한 것입니다(가짜 시계로 레이트리밋·피크시간을 제어). `max_retries`/`retry_max_wait`/`rng` 는 429 재시도 동작을 제어합니다. 일반 사용 시엔 신경 쓸 필요 없습니다.
...
4. **언래핑** — `200` 이면 응답 헤더로 버킷 동기화 후 `resp.json()["result"]` 를 반환(토큰 엔드포인트 제외). 바디가 **비-JSON·과도하게 중첩된 JSON(`RecursionError`)이거나 `result` 키가 없으면** `TossInvestError`(`invalid-response` / `missing-result`)로 거부 — `None` 을 조용히 순회하다 `TypeError` 로 깨지지 않게.
...
try:
    ...
    # SDK 가 max_retries 횟수만큼 자동 재시도 후 소진 시 raise — 필요 시 추가 처리
    ...
