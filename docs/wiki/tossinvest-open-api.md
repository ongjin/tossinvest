...
> 📌 **이 레포에서의 구현 현황**: SDK(`pytossinvest`)는 §1~§5 의 인증·레이트리밋·decimal·에러·엔드포인트를 구현 완료(MIT). MCP 서버(`pytossinvest-mcp`)는 그 위에 안전모델(모드·가드레일·preview→confirm·멱등성)을 얹음(Apache-2.0). 설계·구현 상세는 `docs/superpowers/` 의 spec/plan, 운영 컨벤션은 루트 `AGENTS.md` 참고.
